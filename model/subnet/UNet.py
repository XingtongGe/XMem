...
# 相当于一个完整的UNet裁剪了一次下采样和上采样，输入和输出为通道数64的feature
class UNet(nn.Module):
    def __init__(self):
        super(UNet, self).__init__()
        out_channels=[64, 128, 256, 512, 1024]
        #下采样
        self.d1=DownsampleLayer(out_channels[0],out_channels[1])# 64-128
        self.d2=DownsampleLayer(out_channels[1],out_channels[2])# 128-256
        self.d3=DownsampleLayer(out_channels[2],out_channels[3])# 256-512
        #上采样
        self.u1=UpSampleLayer(out_channels[3],out_channels[3])# 512-1024-1024-512->concat->1024
        self.u2=UpSampleLayer(out_channels[4],out_channels[2])# 1024-512-512-256->concat->512
        self.u3=UpSampleLayer(out_channels[3],out_channels[1])# 512-256-256-128->concat->256
        # self.u4=UpSampleLayer(out_channels[2],out_channels[0])# 256-...->128
        #输出
        self.o=nn.Sequential(
